...
(defn stnd-hdrs [{res-hmap :headers}]
  (let [hvec (apply dissoc res-hmap simple-hdrs);remove simple headers and add remaing as comma delimited list to expose header
        hstr (->> hvec keys (join ", ")) ]
...
(defn allow-origins [req handler & [allow :as orig-regxps]]
  (let [req-hmap (req :headers)
    ...
    (let [resp (handler req)]
      (cond pflt-ok? (-> resp (assoc :status 200) (mergeh (pflt-hdrs req-hmap)));if options req and from allowed origin ;change status to 200, merge resp with pflt and cors headers after removing nils
            cors-ok? (->> resp ((juxt identity stnd-hdrs)) (apply mergeh));if from allowed origin add expose headers to resp, probably use this as generic handler
            :else    resp))))
